...
class App {
  ...
  async startExpress() {
    ...
    this.server.keepAliveTimeout = this.config.app.keepAliveTimeout
    this.server.headersTimeout = this.config.app.headersTimeout
    this.server.requestTimeout = this.config.app.requestTimeout

    console.log(`##### app listening to Port ${this.config.app.port}... #####`)
    console.log('##### Server info')
    console.log(`##### keepAliveTimeout: ${this.server.keepAliveTimeout}`)
    console.log(`##### headersTimeout: ${this.server.headersTimeout}`)
    console.log(`##### requestTimeout: ${this.server.requestTimeout}`)
  }
  ...
  async start() {
    ...
    log('worker', 'trace', {
      message: `before sigterm`,
    });

    process.on('SIGTERM', async () => {
      log('worker', 'trace', {
        message: `sl-express received sigterm`,
      });
      // this.stop.bind(this);
    ...
    log('worker', 'trace', {
      message: `in middle sigterm`,
    });

    process.on('SIGINT', async () => {
      log('worker', 'trace', {
        message: `sl-express received sigint`,
      });
      // this.stop.bind(this);
